//! BPF integration tests for the ac-enforcer.
//!
...
//! - The compiled `ac-ebpf` ELF (built automatically by `aya-build` during `cargo build`)
//!
...
use ac_enforcer::bpf::BpfPolicyManager;
use ac_enforcer::policy::{
...
async fn test_network_apply_egress_rule() {
    use ac_enforcer::policy::EgressRule;

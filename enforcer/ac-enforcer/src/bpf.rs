...
mod linux {
    ...
    use ac_common::events as bpf_events;
    use ac_common::maps::{
    ...
    const BPF_ELF_PATHS: &[&str] = &[
        "/usr/lib/ac-enforcer/ac-ebpf-progs",
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../target/bpfel-unknown-none/release/ac-ebpf-progs"
        ),
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../target/bpfel-unknown-none/debug/ac-ebpf-progs"
        ),
    ];
    ...
    impl BpfPolicyManager {
        ...
        /// 3. `/usr/lib/ac-enforcer/ac-ebpf` (installed path)
        /// 4. `target/bpfel-unknown-none/release/ac-ebpf` (xtask release build)
        /// 5. `target/bpfel-unknown-none/debug/ac-ebpf` (xtask debug build)
        ///
        /// This requires:
        /// - Linux 5.15+ with BTF support
        /// - CAP_BPF + CAP_NET_ADMIN capabilities
        /// - The `ac-ebpf` crate compiled to BPF bytecode (automatic via `cargo build`)
        ...
        fn load_bpf_elf() -> anyhow::Result<Vec<u8>> {
            ...
            if let Some(out_dir) = BPF_ELF_AYA_BUILD {
                let path = format!("{out_dir}/ac-ebpf-progs");
            ...
            anyhow::bail!(
                "BPF ELF not found. Run `cargo build` (aya-build compiles it automatically) \
                 or set {BPF_ELF_ENV} to the path of the compiled ac-ebpf binary. \
                 Searched paths: {:?}",
                BPF_ELF_PATHS,
            )
        }

//! ac-enforcer: Userspace BPF enforcement daemon for agentcontainers.
//!
//! This binary:
...
fn generate_session_certs(creds_dir: &str) -> anyhow::Result<SessionCerts> {
    ...
    ca_dn.push(DnType::CommonName, "ac-enforcer ephemeral CA");
...
use ac_common::bundle::PolicyBundle;
use ac_enforcer::{bpf, grpc, policy};

/// ac-enforcer — BPF enforcement daemon for agentcontainers.
#[derive(Parser, Debug)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let _otel_provider = {
        ...
        if let Some(ref endpoint) = args.otlp_endpoint {
            ...
            let otel_layer =
                tracing_opentelemetry::layer().with_tracer(provider.tracer("ac-enforcer"));
    ...
    if let Some(ref provider) = _otel_provider {
        ac_enforcer::telemetry::shutdown_tracer_provider(provider);
    }

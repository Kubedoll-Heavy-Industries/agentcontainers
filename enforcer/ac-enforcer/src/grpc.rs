...
use ac_common::bundle::{BundleEgressRule, PolicyBundle};
...
impl Enforcer for EnforcerService {
    ...
    async fn apply_credential_policy(
        ...
        // Bundle ACL re-derivation: reject if request exceeds signed policy baseline.
        if let Some(bundle) = &self.policy_bundle {
            let bundle_acls: Vec<ac_common::bundle::BundleSecretAcl> = req
                .secret_acls
                .iter()
                .map(|a| ac_common::bundle::BundleSecretAcl {
                    path: a.path.clone(),

...
fn main() -> anyhow::Result<()> {
    ...
    // Compile ac-ebpf BPF programs using aya-build.
    //
    // The ac-ebpf crate is NOT a workspace member (it targets bpfel-unknown-none),
    ...
    let ebpf_dir = format!("{}/../ac-ebpf", env!("CARGO_MANIFEST_DIR"));
    ...
    if skip {
        ...
        aya_build::build_ebpf([ebpf_package], aya_build::Toolchain::default())
            .context("failed to build ac-ebpf BPF programs")?;

...
fn try_file_open(ctx: &LsmContext) -> Result<i32, i64> {
    ...
    if unsafe { is_proc_environ(file_ptr) } {
        bump_fs_stat(ac_common::events::STAT_FS_BLOCKED);
    ...
    if dentry_ptr.is_null() {
        bump_fs_stat(ac_common::events::STAT_FS_ALLOWED);
    ...
    if let Some(acl) = unsafe { SECRET_ACLS.get(&cred_key) } {
        ...
        if acl.expires_at_ns > 0 && now_ns > acl.expires_at_ns {
            // Expired — deny.
            bump_cred_stat(ac_common::events::STAT_CRED_BLOCKED);
        ...
        bump_cred_stat(ac_common::events::STAT_CRED_ALLOWED);
